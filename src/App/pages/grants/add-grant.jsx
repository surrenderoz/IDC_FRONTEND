...
const AddGrant = () => {
    ...
    return <Container fluid  >
        <Row style={{ display: 'flex', width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 4 }} >
            {BUTTONs?.map((item, i) => <Button key={i.toString()} style={{ flex: 1 }} variant={selectedBtn === i ? "primary" : 'secondary'} onClick={() => setSelectedBtn(i)} >{item}</Button>)}
        </Row>
        <UiForm   >
            <Container style={{ padding: 5, gap: 5 }} >
                {
                    FORMs?.[BUTTONs[selectedBtn]].map((rowsList) => <Row >
                        {
                            rowsList.map((form, formKey) => <Col key={form?.name || formKey.toString()} >
                                {
                                    form.label && <UiForm.Group className="mb-3" id={form.name} controlId={form.name} >
                                        <UiForm.Label>{form.label}</UiForm.Label>
                                        <UiForm.Control min={0} type={form.type} id={form.name} name={form?.name} value={formInputs[form.name]} onChange={e => onChange(form?.name, e.target.value)} placeholder={form.placeholder || ""} />
                                    </UiForm.Group>
                                }</Col>
                            )
                        }
        ...
        <AnimatedModal animation={'zoom'} visible={showModal} onClose={() => setShowModal(false)}>
            ...
            <Card.Body>
                <Card.Title className='text-center' as="h5">Grant Name: <span style={{ color: 'dodgerblue' }} >{formInputs?.grantName}</span> <br /> <br /> added <span style={{ color: 'green' }} >succefully!!</span> </Card.Title>
            </Card.Body>
